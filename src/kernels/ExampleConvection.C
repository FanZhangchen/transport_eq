...
registerMooseObject("transportEQApp", ExampleConvection);
...
ExampleConvection::ExampleConvection(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _velocity(getMaterialProperty<Real>("convection_velocity"))
{
}
...
ExampleConvection::computeQpResidual()
{
  ...
  return _test[_i][_qp] * (_velocity[_qp] * _grad_u[_qp](0));
}

Real
ExampleConvection::computeQpJacobian()
{
  return _test[_i][_qp] * (_velocity[_qp] * _grad_phi[_j][_qp](0));
}
